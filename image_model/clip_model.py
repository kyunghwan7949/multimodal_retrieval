...
from tokengt.modules.tokenizer import GraphFeatureTokenizer as GT
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.attn = nn.MultiheadAttention(d_model, n_head).to(dtype=torch.float16)
    ...
    def forward(self, x: torch.Tensor):
        ...
        q = self.ln_1(x)  # Apply LayerNorm to generate q tensor
        k = self.ln_1(x)  # Apply LayerNorm to generate k tensor
        v = self.ln_1(x)  # Apply LayerNorm to generate v tensor

        x = x + self.attention(q, k, v)  # Pass q, k, v tensors to attention
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.lap_node_id_k =16
        self.lap_node_id_eig_dropout = 0.2
        self.lap_node_id_sign_flip = True
        self.lap_encoder = nn.Linear(2 * self.lap_node_id_k, transformer_width, bias=False)
        self.lap_eig_dropout = nn.Dropout2d(p=self.lap_node_id_eig_dropout) if self.lap_node_id_eig_dropout > 0 else None
        self.graph_token = nn.Embedding(1, transformer_width)
    ...
    def encode_graph(self, graph, training=True, mlm=False, sot_token=49406, eot_token=49407):

        if mlm:
            node_data = graph["mlm_nodes"]
            edge_data = graph["mlm_edges"]
        else:
            node_data = graph["node_data"]
            edge_data = graph["edge_data"]

        node_num = graph["node_num"]
        lap_eigvec = graph["lap_eigvec"]
        lap_eigval = graph["lap_eigval"]
        edge_index = graph["edge_index"]
        edge_num = graph["edge_num"]
        # sot_token = graph["sot_token"].unsqueeze(1)

        node_feature = self.token_embedding(node_data).type(self.dtype)  # [batch_size, n_ctx, d_model]
        edge_feature = self.token_embedding(edge_data).type(self.dtype)

        padded_index, padded_feature, padding_mask, _, _ = GT.get_batch(
            node_feature, edge_index, edge_feature, node_num, edge_num
        )
        node_mask = GT.get_node_mask(node_num, node_feature.device)  # [B, max(n_node)]
        ...
        if training:
            sign_flip = GT.get_random_sign_flip(eigvec, node_mask)
            lap_node_id = eigvec * sign_flip
        else:
            lap_node_id = eigvec

        lap_index_embed = GT.get_index_embed(lap_node_id, node_mask, padded_index)  # [B, T, 2Dl]
        padded_feature = padded_feature + self.lap_encoder(lap_index_embed)
        graph_feature = padded_feature.masked_fill(padding_mask[..., None], float('0'))
        
        b, _, d = padded_feature.size()
        sot_token = torch.tensor([sot_token]).to(node_feature.device)
        eot_token = torch.tensor([eot_token]).to(node_feature.device)

        sot_feature = self.token_embedding(sot_token).type(self.dtype).expand(b, 1, d)  # [1, D]
        eot_feature = self.token_embedding(eot_token).type(self.dtype).expand(b, 1, d)  # [1, D]

        x = torch.cat((sot_feature, graph_feature, eot_feature), dim=1)  # [B, 2, D]
