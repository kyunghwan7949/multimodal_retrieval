...
class RetrievalGraphDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(self.annotation[index].values())
        ...
        mlm_node, mlm_edge, mlm_label = self._build_random_masked_tokens_and_labels_edge_first(node_input_ids, edge_input_ids, edge_index.t(), self.tokenizer)
    ...
    def _build_random_masked_tokens_and_labels(self, node_input_ids, edge_input_ids, edge_index, tokenizer):
        ...
        token_range = list(range(4, len(tokenizer.encoder)-1)) # 4 ~ 26962
        ...
        for i, token in enumerate(node_input_ids):
            ...
            if 3 < token < 3083:
                ...
                # 15% masking 토큰 중 80% 토큰은 "<mask>"로 masking
                if prob < 0.15:
                    prob /= 0.15
                    ...
                    # 15% masking 토큰 중 10% 토큰은 임의 토큰으로 masking
                    elif prob < 0.9:
                    ...
                    # 15% masking 토큰 중 나머지 10% 토큰은 그대로 토큰 유지

                    # append current token to output (we will predict these later)
                    node_labels.append(token)
                else:
                    # 마스킹 되지 않은 노드들의 레이블은 0으로 초기화
    ...
    '''
    def _build_random_masked_tokens_and_labels_edge_first(self, node_input_ids, edge_input_ids, edge_index, tokenizer):
        mask = tokenizer.encoder["<mask>"]
    ...
    #     for i, token in enumerate(edge_input_ids):
    #     # for i, token in enumerate(edge_input_ids):
    #         if 3 < token < 26963:
    #             prob = random.random()
    ...
    #                 # (노드A, 노드B)엣지와 (노드B, 노드A)엣지를 서로 다른 엣지로 간주
    #                 edge_key = (edge_index[i][0].item(), edge_index[i][1].item())
    #                 edge_key_reverse = (edge_key[1], edge_key[0])

    #                 if edge_key not in masked_edges and edge_key_reverse not in masked_edges:
    #                     masked_edges[edge_key] = True
    #                     masked_edges[edge_key_reverse] = True
    #                     edge_labels[i] = eids[i]

    #                     # 15% masking 토큰 중 80% 토큰은 "<mask>"로 마스킹
    #                     if prob < 0.8:
    #                         eids[i] = mask  #  엣지 마스킹

    #                         node_indices = [
    #                             idx for idx in edge_key if nids[idx] != mask and idx not in masked_nodes
    #                         ]

    #                         for node_index in node_indices:
    #                             node_labels[node_index] = nids[node_index]
    #                             masked_nodes[node_index] = True
    #                             nids[node_index] = mask  # 노드 마스킹

    #                     # 15% masking 토큰 중 10% 토큰은 임의 토큰으로 마스킹
    #                     elif prob < 0.9:
    #                         eids[i] = random.choice(token_range)
    ...
    #     print("Masked edges: ", masked_edges.keys())  # 마스킹된 엣지 출력
    #     print("Nodes connected to masked edges: ", masked_nodes.keys())  # 마스킹된 엣지와 연결된 노드 출력
    #     print("-----------")
...
class RetrievalGraphEvalDataset(Dataset):
    ...
    def _build_random_masked_tokens_and_labels(self, node_input_ids, edge_input_ids, edge_index, tokenizer):
        ...
        padded_labels = torch.zeros(self.max_length, dtype=torch.long) # node_labels와 edge_labels를 결합하여 labels tensor를 생성하고, 이를 max_length까지 패딩
    ...
    '''
    마스킹 조건
    조건1. (노드A, 노드B)를 가지는 엣지와 (노드B, 노드A)를 가지는 엣지는 서로 다른 엣지로 간주하여 엣지 마스킹한다. 
    조건2. (노드A, 노드B) 엣지에서 엣지 마스킹 진행 후, 노드A와 노드B를 마스킹하게 되면, (노드B, 노드A) 엣지에서 엣지 마스킹 진행 후, 노드A와  노드B는 이미 앞서 노드 마스킹 하였으므로 하지 않는다.
    조건3. 마스킹된 엣지와 연결된 노드들은 모두 마스킹한다.
    '''
